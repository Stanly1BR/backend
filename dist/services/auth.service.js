...
export class AuthService {
    async Login(authData) {
        ...
        if (user.tipo === 'paciente') {
            ...
            if (perfilPaciente)
                userId = perfilPaciente.id;
        }
        else if (user.tipo === 'medico') {
            const perfilMedico = await medico.findOne({ where: { authId: user.id } });
            if (perfilMedico)
                userId = perfilMedico.id;
        }
        const token = jwt.sign({ userId, tipo: user.tipo }, JWT_SECRET, { expiresIn: '1d' });
    ...
    async Register(authData) {
        ...
        const token = jwt.sign({ userId, tipo: newAuth.tipo }, JWT_SECRET, { expiresIn: '1d' });
